---
title: "CAWS: CUT&Tag Analysis Report"
output:
...
if (length(as.character(snakemake@input[["tss_enrichment"]])) > 0) {
  ...
  if (file.exists(tss_path)) {
    tss_enrichment <- read_data(tss_path) |>
        mutate(sample_id = str_replace_all(sample_id, "_raw", ""))
  }
}
```
...
if (nrow(consolidated) > 0) {
  ...
  if (!is.null(tss_enrichment) && nrow(tss_enrichment) > 0) {
    summary_stats <- left_join(summary_stats, tss_enrichment, by = c("sampleID" = "sample_id")) |> distinct()
  }
...
process_coverage_files <- function(coverage_files) {
  ...
  # Read the first file to establish bin coordinates and sample row indices upfront.
  first_dt <- tryCatch(
    fread(coverage_files[[1]], col.names = c("chrom", "start", "coverage")),
    error = function(e) NULL
  )
  if (is.null(first_dt) || nrow(first_dt) == 0) return(tibble())

  # Subsample before any merging to avoid building a genome-wide table.
  n_bins    <- nrow(first_dt)
  sample_idx <- if (n_bins > 50000) sort(sample.int(n_bins, 50000)) else seq_len(n_bins)

  first_name <- str_remove(basename(coverage_files[[1]]), ".bin.bed")
  result <- data.table(
    chrom = first_dt$chrom[sample_idx],
    bin   = paste(first_dt$chrom[sample_idx], first_dt$start[sample_idx], sep = "_")
  )
  result[, (first_name) := first_dt$coverage[sample_idx]]

  # All bin BED files share the same bin order (same genome, same pipeline step),
  # so read only the coverage column for remaining files and cbind — no join needed.
  for (f in coverage_files[-1]) {
    sname <- str_remove(basename(f), ".bin.bed")
    cov <- tryCatch(
      fread(f, select = 3L, col.names = "coverage")$coverage[sample_idx],
      error = function(e) rep(NA_real_, length(sample_idx))
    )
    result[, (sname) := cov]
  }
...
create_peak_width_plot <- function(peak_width_data, sample_data, method_name, bins = hist_bins) {
  ...
  plot_data |>
    ...
    scale_x_continuous(trans = "log10", labels = ~format(., scientific = FALSE)) +
...
seacr_peak_files <- as.character(snakemake@input[["seacr_peaks"]]) |> map_chr(get_abs_path) |> keep(~ file.exists(.x) && file.info(.x)$size > 0)
...
# some MACS3 se peak files can be empty since a model was not built.
macs3_peak_files <- as.character(snakemake@input[["macs3_peaks"]]) |> map_chr(get_abs_path) |> keep(~ file.exists(.x) && file.info(.x)$size > 0)
