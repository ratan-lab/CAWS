...
stats <- read_tsv(get_abs_path(as.character(snakemake@input[["stats"]])))
seacr_num <- read_tsv(get_abs_path(as.character(snakemake@input[["seacr_num"]])))
seacr_reprod <- read_tsv(get_abs_path(as.character(snakemake@input[["seacr_reprod"]])))
seacr_frip <- read_tsv(get_abs_path(as.character(snakemake@input[["seacr_frip"]])))
seacr_width <- read_tsv(get_abs_path(as.character(snakemake@input[["seacr_width"]])))
macs3_num <- read_tsv(get_abs_path(as.character(snakemake@input[["macs3_num"]])))
macs3_reprod <- read_tsv(get_abs_path(as.character(snakemake@input[["macs3_reprod"]])))
macs3_frip <- read_tsv(get_abs_path(as.character(snakemake@input[["macs3_frip"]])))
macs3_width <- read_tsv(get_abs_path(as.character(snakemake@input[["macs3_width"]])))
consolidated <- read_tsv(get_abs_path(as.character(snakemake@input[["consolidated"]])))
...
if (length(as.character(snakemake@input[["tss_enrichment"]])) > 0) {
  ...
  if (file.exists(tss_path)) {
    tss_enrichment <- read_tsv(tss_path)
  }
}
```
...
for (relname in as.character(snakemake@input[["seacr_peaks"]])) {
  absname <- get_abs_path(relname)
  if (file.exists(absname) && file.size(absname) > 0) {
...
for (relname in as.character(snakemake@input[["macs3_peaks"]])) {
  absname <- get_abs_path(relname)
  if (file.exists(absname) && file.size(absname) > 0) {
    filename <- basename(absname)
...
fragment_files <- as.character(snakemake@input[["fragments"]])
fragment_data <- tibble()

for (rel_file in fragment_files) {
  abs_file <- get_abs_path(rel_file)
  if (file.exists(abs_file)) {
    sample <- str_extract(basename(abs_file), "^[^.]+")
    frags <- read_tsv(abs_file, col_names = c("length", "count"))
