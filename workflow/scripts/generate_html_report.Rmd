...
consolidated <- read_tsv(as.character(snakemake@input[["consolidated"]]))

# Read TSS enrichment if available
tss_enrichment <- NULL
if (length(as.character(snakemake@input[["tss_enrichment"]])) > 0 &&
    file.exists(as.character(snakemake@input[["tss_enrichment"]]))) {
  tss_enrichment <- read_tsv(as.character(snakemake@input[["tss_enrichment"]]))
}
```
...
for (absname in as.character(snakemake@input[["seacr_peaks"]])) {
...
for (absname in as.character(snakemake@input[["macs3_peaks"]])) {
  if (file.size(absname) > 0) {
    filename <- str_split(absname, "/")[[1]][3]
